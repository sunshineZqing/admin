		...
		<div class="addtions">
			...
			<div>
				<el-select style='width: 125px;margin-right:10px;' v-model="baseinfoParams.fil" slot="prepend" placeholder="时间" clearable>
					...
					<el-option label="转催收时间" value="2"></el-option>
			...
			<a v-if="downloadPower == 1 || downloadPower == 24" class="download" :href="'/ykj/collectionRepayment/listJson/excel.do?keywords='+baseinfoParams.keywords+'&collectionPeople='+baseinfoParams.collectionPeople+'&fil='+baseinfoParams.fil+'&startTime='+baseinfoParams.startTime+'&endTime='+baseinfoParams.endTime" target="_blank">下载</a>
			
		</div>
		<basetable :showopera='true' :loading='loading' :basetabletitle="basetabletitle" :basetableinfo="baseinfo" key="userinfo">
...
<script>
	...
	export default {
		...
		methods: {
			init() { //初始化
				this.loading = true;
				this.$api.rechargerecords(this.baseinfoParams).then(res => {}, response => {
					this.loading = false
