	...
	<div class="riskdetail">
		...
		<transition name="fade">		
			<div class="authsuggestions" id="dragdiv" draggable="true" v-show="authsuggestions">
				...
				<toexamineseconed></toexamineseconed>
			</div>
	    </transition>-->
			...
			<div class="renzheng-content">
				...
				<div class="renzheng-item">
				    ...
				    <div class="renzhengItemRight">
						<toexamineseconed :params = "params3"></toexamineseconed>
					</div>
				</div>
			</div>
		</div>
...
<script>
	...
	export default{
		components: {tabpanel,toexamineseconed,sliderright},
		data(){
			return{
				userinfo:{
                    'name':'',
...
//					au_id:1271615,
	            ...
	            this.$api.getriskprimaryuserinfo(params).then(res=>{},response=>{
	            	if(response.data.resultCode==0){
		                ...
		                if(response.data.returnObj.certifyList.length != 0){
...
//		                })
//		               console.log(item,55)
		                this.getauthiteminfo(this.realtags[0].ordernum,this.realtags[0].au_id)
	            	}                                
		        }).catch(val=>{//捕获异常
		        	console.log(val,44)
			...
			getauthsugg(){ //审核建议数据
				...
				this.$api.authsuggests(params).then(res=>{},response=>{//请求数据
                	...
                	if(response.data.resultCode=='0'){
                        ...
                        let concatPd = ''
                        if(response.data.returnObj.concatPd.length !=0){
...
//                  au_ynumauth_type:ordernum,
                ...
                this.$api.getauthitem(authinfo).then(res=>{},response=>{
                    ...
                    this.$message({type:'error',message:'服务器异常2'})
		        }) 
            },
		}
	}
