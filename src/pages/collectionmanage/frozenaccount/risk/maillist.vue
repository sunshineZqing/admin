				...
				<el-tab-pane v-for="item in roleList" :label="item.name" :name="item.name" :key="item.name">
					...
					<div v-show="item.name=='联系人'">
						...
						<div class="">
							...
							<div class="mailer">
								<origintable  :originTableTitel="urgentmailertitle" :originTableInfo="urgentmailerinfo"></origintable>
							</div>	
						</div>
	...
	export default {
		...
		data(){
			return{
                ...
                //紧急联系人
                urgentmailertitle:[
	                {   label:'紧急联系人姓名', prop:'phone_name'},  
	                {   label:'紧急联系人手机号',   prop:'phone'},
	                {   label:'备注', prop:'atd_remark'},  
                ],
                urgentmailerinfo:[],
                summarypeople:0, //总人数
		...
		methods:{
			showdetails(){//原始数据
				if(!this.isshow){
					...
					this.$fksh.getoriginalhighRiskMaillist({userId:this.$route.params.user_id}).then(res=>{},response=>{ //高危联系人
	        ...
	        getmaillist(){
                this.$fksh.getoriginalMaillist({userId:this.$route.params.user_id}).then(res=>{},response=>{//联系人
	            	...
	            	if(response.data.resultCode=='0'){
		            	...
		            	if(response.data.returnObj.conlist){
		            		this.urgentmailerinfo = response.data.returnObj.conlist;
		            	}
	            	}else{
	            		this.contantload = false
	            		this.$message.error('查询失败')
