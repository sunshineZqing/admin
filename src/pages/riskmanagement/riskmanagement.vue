	...
	<div>
		...
		<transition name="fade">		
			<div class="authsuggestions" id="dragdiv" draggable="true" v-show="authsuggestions">
				...
				<toexamine v-if="action==1"></toexamine>
				<toexamineseconed v-if="action==2"></toexamineseconed>
				<toexaminelast v-if="action==3"></toexaminelast>
			</div>
	    </transition>-->
			...
			<div class="renzheng-content">
				...
				<div class="renzheng-item">
					...
					<div class="renzhengItemRight">
						<toexamine v-if="action==1"></toexamine>
						<toexamineseconed v-if="action==2" :params = "params3"></toexamineseconed>
						<toexaminelast v-if="action==3"></toexaminelast>
					</div>
				</div>
			</div>
...
<script>
    ...
    import toexamine from './risk/toexamine/toexamine'
	import toexamineseconed from './risk/toexamine/toexamineseconed'
	import toexaminelast from './risk/toexamine/toexaminelast'
	...
	export default {
		 beforeRouteEnter (to, from, next) {
	      next(vm => {
	      	    switch(from.name){
	      	   	  case '初审':vm.currentpageid = 1 ;break;
	      	   	  case '复审':vm.currentpageid = 1 ;break;
	      	   	  case '终审':vm.currentpageid = 1 ;break;
	      	   	  case  null :vm.currentpageid = 1 ;break;// 刷新为null
	      	   	  case  '系统提额复审' :vm.currentpageid = 2 ;break;
	      	   	  case  '系统提额终审' :vm.currentpageid = 2 ;break;
	      	   	  default:vm.currentpageid  = 0 //需进行存储
	      	    }
	      })
	    },
	    mounted(){
...
////	    	var sliderfight = document.querySelector('#sliderfight');
...
////				sliderfight.style.left = (e.clientX - x) + 'px';
////				
////				sliderfight.style.top  = (e.clientY - y)+300 + 'px';
//
...
//			}, false);
            ...
            this.$store.dispatch('setaction',this.$route.params.action)
      	    this.getlist();
      	    this.getauthsugg()//显示审核建议框，且加载数据
        ...
        computed:{
            ...mapGetters(['score','ma_id','user_id','pageid','authsuggestions'])
		},
		components:{tabpanel,toexamine,toexamineseconed,toexaminelast,sliderright},
		methods:{
			getlist(){
	            let params = {
	            ...
	            this.$api.getriskprimaryuserinfo(params).then(res=>{},response=>{
	            	if(response.data.resultCode=='0'){
		                ...
		                if(!isEmpty(response.data.returnObj.certifyList)){
			                this.au_id    = response.data.returnObj.certifyList[0].au_id
			                this.au_num   = response.data.returnObj.certifyList[0].au_num
...
//                      ]
                        ...
                        this.$store.dispatch('TOGETSUBMIT',this.params3); //存储数据
	                }else{
