...
<script>
	...
	export default{
		name:'jd',
		components:{cloudsubmit,basetable},
		props:{
		...
		data(){
			return {
                ...
                newReOrderListtitle:[
                  { label:'订单总金额', prop:'amount',width:'150px'},  
                  { label:'消费时间',   prop:'businessTime',width:'150px'},  
                  { label:'消费者姓名', prop:'customerName',width:'150px'},
                  { label:'订单地址', prop:'orderAddr',width:'150px'},
                  { label:'订单电话', prop:'orderPhone',width:'150px'},
                  { label:'订单状态', prop:'orderStatus',width:'150px'},
                  { label:'支付方式', prop:'payType',width:'150px'}
                ],
                newReOrderListinfo:[],
                reOrderSize:'',//订单总量
		...
		methods:{
			showdetails(){//原始数据
				if(!this.isshow){
	                ...
	                this.$fksh.getoriginaljd({user_id:this.$route.params.user_id,type:sessionStorage.getItem('newlodType')}).then(res=>{},response=>{
		            	...
		            	if(response.data.resultCode=='0'){
		            		if(!isEmptyObject(response.data.returnObj)){
			            		...
			            		//收货地址
			            		if(response.data.returnObj.addressList.length != 0){
			            		...
			            		//今年订单记录
			            		if(response.data.returnObj.newReOrderList.length != 0){
			            		...
			            		//去年订单记录
	                            if(response.data.returnObj.newPreOrderList.length != 0){
