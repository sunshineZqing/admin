				...
				<el-tab-pane v-for="item in roleList" :label="item.name" :name="item.name" :key="item.name">
					...
					<div v-show="item.name=='收货地址'">
						<div class="tbtaginfoout">
							<el-tag  type="danger"><span>数量：</span>{{addressnum}}</el-tag>
						</div>
					...
					<div v-show="item.name=='订单详情'">
						<div class="tbtaginfoout">
							<el-tag  type="danger"><span>订单总量：</span>{{orderNum}}</el-tag>
							<el-tag  type="danger"><span>有效订单总量：</span>{{validOrderSize}}</el-tag>
							<el-tag  type="danger"><span>有效订单总额：</span>{{orderPriceAll}}</el-tag>
							<el-tag  type="danger"><span>平均每笔消费：</span>{{orderPriceAVG}}</el-tag>
						</div>
						<br />
						<div class="tableStyle">
							<div class="tablestyleTitle">
								...
								<div v-for="item in ordertitle" class="tablestyleTitleItem" :style="{width:item.width}">
							...
							<div class="tablestyleBody"  v-if="orderinfo.length != 0">
								<div class="tablestyleBodyLine" v-for="(item,index) in orderinfo">
									<div class="tablestyleBodyLineItem">
										...
										<div class="" v-for="item2 in ordertitle" :style="{width:item2.width}">
											<span>{{item[item2.prop]}}</span>
										</div>
									</div>
									<div class="tablestyleBodyLinedetail">
...
<script>
	...
	export default{
		name:'taobao',
		components:{cloudsubmit,basetable,basepage},
		props:{
		...
		data(){
            return {
                ...
                addressnum:0,
                ordertitle:[
                  {   label:'订单编号', prop:'orderid',width:'260px'},  
                  {   label:'订单日期', prop:'businessDate',width:'260px'},  
                  {   label:'订单总额', prop:'orderTotalPrice',width:'260px'},
                  {   label:'订单状态', prop:'orderStatus',width:'260px'}
                ],
                orderinfo:[],
                orderinfosave:[],
                orderinfopagesize:0,
		...
		methods:{
			showdetails(){//原始数据
				if(!this.isshow){
	                ...
	                this.$fksh.getoriginaltaobao({user_id:this.$route.params.user_id,type:sessionStorage.getItem('newlodType')}).then(res=>{},response=>{
	                	this.contantload = false
		            	if(response.data.resultCode=='0'){
		            		if(!isEmptyObject(response.data.returnObj)){
		            			//基本信息
				            	if(!isEmptyObject(response.data.returnObj.personInfo)){
				            	...
				            	//收货地址
			            		if(!isEmptyObject(response.data.returnObj.addressList)){
			            		...
			            		if(!isEmptyObject(response.data.returnObj.newOrderList)){
