...
//			        })
					...
					this.$api.mailBack({user_id:this.$route.params.user_id,type:sessionStorage.getItem('newlodType')}).then(res=>{},response=>{
						this.contantload = false
						if(response.data.resultCode == 0){
								...
								for(var i =0 ; i<this.roleList.length; i++){
									for(var j = 0;j<this.roleList[i].monthList.length;j++){
									    ...
									    this.roleList[i].monthList[j].flowList.forEach(elem=>{
									    	console.log(fqreg.test(elem.des),lxreg.test(elem.des),njreg.test(elem.des), wyjreg.test(elem.des) , yjreg.test(elem.des) , sxfreg.test(elem.des))
